...
input double InpMinBuyScore           = 6.5;   // OPT 4: subido de 5.0 a 6.5
input double InpMinSellScore          = 6.5;   // OPT 4: subido de 5.0 a 6.5
...
input double InpTPPips = 0;          // 0 = auto (1.5x SL)
input double InpSLPips = 0;          // 0 = auto (1.5x ATR)
input double InpSL_ATR_Multiplier = 1.5;  // OPT 3: multiplicador ATR para SL
input double InpTP_SL_Ratio = 1.5;        // OPT 3: ratio TP:SL
...
input int    InpPanelScalePercent     = 100;   // 100=normal, 75=compacto laptop, 125=grande
input int    InpPanelUpdateSeconds    = 3;     // segundos entre updates (reducir lag)
input bool   InpPanelCornerRight      = false; // true = esquina derecha
...
// OPT 1-2: Panel responsive - variables de escala
...
//+------------------------------------------------------------------+
//  OPT 5: FILTRO DE MOMENTUM + VOLATILIDAD (Anti-Chop ligero)
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//  TRAILING + QUICK CAPTURE — FIX 1 + FIX 3
//+------------------------------------------------------------------+
void UpdateTrailing()
{
   if(g_basketActive) return;
...
//+------------------------------------------------------------------+
//  BASKET RECOVERY — 6 fases — FIX 2 en FASE 5
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//  OPT 3: OPEN ORDER CON SL INTELIGENTE BASADO EN ATR
//+------------------------------------------------------------------+
bool OpenOrder(ENUM_ORDER_TYPE dir,double score)
{
   ...
   // OPT 3: SL INTELIGENTE
   double atrVal = GetCurrentATR();
   double sl=0, tp=0;

   double sld=0;
   if(InpSLPips>0)
      sld = PipsToPrice(InpSLPips);                    // SL manual
   else if(atrVal>0)
      sld = atrVal * InpSL_ATR_Multiplier;            // SL = 1.5x ATR automatico
   else
      sld = PipsToPrice(30.0);                         // fallback 30 pips

   double tpd=0;
   if(InpTPPips>0)
      tpd = PipsToPrice(InpTPPips);
   else if(sld>0)
      tpd = sld * InpTP_SL_Ratio;                      // RR minimo 1:1.5
...
void ManageEntries()
{
   ...
   if(bsc>=bth&&ssc<sth){
      ...
      if(!IsMomentumVolatilityFilter(ORDER_TYPE_BUY)) return;        // OPT 5
      ...
      if(!IsMomentumVolatilityFilter(ORDER_TYPE_SELL)) return;       // OPT 5
...
//+------------------------------------------------------------------+
//  OPT 1-2: PANEL RESPONSIVE Y ULTRA-LIGERO
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//  OPT 1: CALCULAR DIMENSIONES RESPONSIVE DEL PANEL
//+------------------------------------------------------------------+
...
void UpdatePanelData()
{
   ...
   g_pTrail=(InpTrailingMode==TRAILING_RETRACEMENT)?"RETRACEMENT %":"ATR x"+DoubleToString(InpTrailATRMultiplier,1);
   if(g_basketActive) g_pTrail+=" [BASKET]";
   g_pHedge=InpEnableBasketHedge?("ON | MaxL:"+IntegerToString(InpMaxHedgeLevels)):"OFF";
   ...
   SignalStrength bs=GetSignalStrength(ORDER_TYPE_BUY);
   SignalStrength ss=GetSignalStrength(ORDER_TYPE_SELL);
   SignalStrength lc; ZeroMemory(lc);
   double lb=ComputeRawScore(ORDER_TYPE_BUY, 0,lc,false);
   double ls=ComputeRawScore(ORDER_TYPE_SELL,0,lc,false);
   double db=MathMax(0.0,MathMin(10.0,bs.finalScore*0.5+lb*0.5));
   double ds=MathMax(0.0,MathMin(10.0,ss.finalScore*0.5+ls*0.5));
   ...
   string tr=IntegerToString(g_tradesToday);
   tr+=(InpMaxTradesPerDay>0)?("/"+IntegerToString(InpMaxTradesPerDay)):"/inf";
   g_pTrades   =tr;
   g_pLoss     =IntegerToString(g_consecutiveLossCount);
   g_pCool     =(g_cooldownUntilBarTime>0)?"ON":"OFF";
...
//+------------------------------------------------------------------+
//  OPT 1-2: UPDATE PANEL RESPONSIVE
//+------------------------------------------------------------------+
void UpdatePanel()
{
   ...
   int hc=26+10;
   hc+=g_secH+g_rowH+g_rowH+22+g_rowH+SB+8+8;
   hc+=g_secH+g_rowH+g_rowH+g_rowH+g_rowH+g_rowH;
   if(g_basketActive) hc+=g_rowH;
   hc+=4+8+8;
   hc+=g_secH+g_rowH+g_rowH+g_rowH+22;
   hc+=8+8;
   hc+=g_secH+g_rowH+g_rowH+g_rowH+22;
   hc+=8+8;
   if(g_buy.hasPosition||g_sell.hasPosition){
      hc+=g_secH+g_rowH+g_rowH;
      if(g_pHealthVal>=0) hc+=g_rowH;
      hc+=g_rowH;
      bool ih=g_buy.hasPosition?g_buy.isHedge:g_sell.isHedge;
      if(ih) hc+=g_rowH;
      if(g_pMins>=0) hc+=SB;
   }
   hc+=22;
   ...
   color trC=g_basketActive?CLR_HEDGE:CLR_BLUE;
   PanelLabel("SGS_L_TR","Trailing:",padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
   PanelLabel("SGS_V_TR",g_pTrail,  colV,row,120,S,trC,fs,"Consolas",corner); row+=g_rowH;
   PanelLabel("SGS_L_HG","Hedge:",   padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
   PanelLabel("SGS_V_HG",g_pHedge,  colV,row,120,S,InpEnableBasketHedge?CLR_GREEN:CLR_NEUTRAL,fs,"Consolas",corner); row+=g_rowH;
   if(g_basketActive&&g_pBasket!=""){
      PanelLabel("SGS_L_BK","Basket:",  padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
      PanelLabel("SGS_V_BK",g_pBasket, colV,row,130,S,CLR_HEDGE,fs,"Consolas",corner); row+=g_rowH;
   }
   row+=4;
   ...
   PanelLabel("SGS_SEC4","= RIESGO",padL,row,120,S,CLR_GOLD,fs,"Consolas",corner); row+=g_secH;
   color trdc=(InpMaxTradesPerDay>0&&g_tradesToday>=InpMaxTradesPerDay)?CLR_RED:CLR_WHITE;
   PanelLabel("SGS_L_TD","Trades:",   padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
   PanelLabel("SGS_V_TD",g_pTrades,  colV,row,80,S,trdc,fs,"Consolas",corner); row+=g_rowH;
   color lsC=(g_consecutiveLossCount>=InpConsecutiveLossesBeforeCooldown)?CLR_RED:CLR_WHITE;
   PanelLabel("SGS_L_LS","Loss str:", padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
   PanelLabel("SGS_V_LS",g_pLoss,    colV,row,80,S,lsC,fs,"Consolas",corner); row+=g_rowH;
   color coC=(g_cooldownUntilBarTime>0)?CLR_RED:CLR_GREEN;
   PanelLabel("SGS_L_CD","Cooldown:", padL,row,80,S,CLR_GRAY,fs,"Consolas",corner);
   PanelLabel("SGS_V_CD",g_pCool,    colV,row,80,S,coC,fs,"Consolas",corner); row+=g_rowH;
   ...
   PanelLabel("SGS_V_SP",g_pSpread,  colV,row,80,S,spC,fs,"Consolas",corner); row+=22;
   PanelLine("SGS_S4",padL,row,padR,row,CLR_BORDER,corner); row+=8;

   if(g_buy.hasPosition||g_sell.hasPosition)
   {
      PanelLabel("SGS_SEC5","= POSICION ACTIVA",padL,row,160,S,CLR_GOLD,fs,"Consolas",corner); row+=g_secH;
      bool ib=g_buy.hasPosition;
      bool ih=ib?g_buy.isHedge:g_sell.isHedge;
      color sdC=ib?(ih?CLR_HEDGE:CLR_BUY):(ih?CLR_HEDGE:CLR_SELL);
      string sdS=ib?(ih?"HEDGE LONG":"LONG"):(ih?"HEDGE SHORT":"SHORT");
      ...
      if(g_pHealthVal>=0){
         ...
         PanelProgress("SGS_BAR_HH",colV+(int)(55*g_scale/100.0),row+4,55,7,g_pHealthVal,hC,CLR_BAR_BG,corner); row+=g_rowH;
      }
      bool tron=ib?g_buy.trailingActive:g_sell.trailingActive;
      PanelLabel("SGS_L_TA","Trailing:",padL,row,70,S,CLR_GRAY,fs,"Consolas",corner);
      PanelLabel("SGS_V_TA",tron?"ACTIVO":"INACTIVO",colV,row,100,S,tron?CLR_GREEN:CLR_NEUTRAL,fs,"Consolas",corner); row+=g_rowH;
      if(ih){
      ...
      if(g_pMins>=0){
         ...
         PanelLabel("SGS_V_MT",IntegerToString(g_pMins)+" min",colV,row,80,S,tmC,fs,"Consolas",corner); row+=SB;
      }
   }
...
void OnTick()
{
   ...
   // OPT 2: Panel update cada N segundos configurable
   datetime now=TimeCurrent();
