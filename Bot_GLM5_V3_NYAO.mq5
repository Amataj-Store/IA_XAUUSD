...
//|                        GLM5 V3 + NYAO Functions (HedgeRequireSignal + Partial Close + News Filter) |
//|                                                                  |
//+------------------------------------------------------------------+
...
//============================================================
//====== NYAO INTEGRATION — NEW INPUTS ======
//============================================================

//====== HEDGE SIGNAL FILTER (Anti-Spike) ======
input group "====== HEDGE SIGNAL FILTER (Anti-Spike) ======"
input bool   InpHedgeRequireSignal  = true;   // Solo hedgear si señal contraria confirma
input double InpHedgeMinSignalScore = 4.5;    // Score mínimo de señal contraria para hedge

//====== PARTIAL CLOSE (Signal Decay) ======
input group "====== PARTIAL CLOSE (Signal Decay) ======"
input bool   InpEnablePartialClose    = true;  // Activar cierre parcial por señal
input double InpPartialCloseL1Signal  = 0.75;  // Cerrar 25% cuando señal baja al 75% del inicial
input double InpPartialCloseL2Signal  = 0.50;  // Cerrar 50% cuando señal baja al 50% del inicial
input double InpPartialCloseL3Signal  = 0.25;  // Cerrar todo cuando señal baja al 25% del inicial

//====== NEWS FILTER (High-Impact Events) ======
input group "====== NEWS FILTER (High-Impact Events) ======"
input bool   InpEnableNewsFilter   = true;   // Pausar trading en noticias de alto impacto
input int    InpNewsMinutesBefore  = 30;     // Minutos antes de la noticia
input int    InpNewsMinutesAfter   = 30;     // Minutos después de la noticia
...
struct ScalperSide
{
   ...
   double    initialScore;      // NYAO: score cuando se abrió la posición
   int       partialCloseLevel; // NYAO: 0=ninguno, 1=L1, 2=L2, 3=cerrado
};
...
int OnInit()
{
   ...
   LogMessage(2, StringFormat("[INIT OK] GLM5_V3_NYAO v3.1 | Symbol=%s | Lot=%.2f | HedgeSignal=%s | PartialClose=%s | NewsFilter=%s",
               _Symbol, InpLotSize,
...
{
   // NYAO FIX: Preserve initialScore and partialCloseLevel across ticks
   double preservedInitialScore = side.initialScore;
   int preservedPartialLevel = side.partialCloseLevel;
   ...
   // Restore preserved values (don't let ResetSide erase them)
   side.initialScore = preservedInitialScore;
   side.partialCloseLevel = preservedPartialLevel;
...
//+------------------------------------------------------------------+
//| CHECK PROFIT LOCK (DO NOT TOUCH)                                 |
//+------------------------------------------------------------------+
void CheckProfitLock(ScalperSide &side)
{
   ...
   // NYAO FIX: Detect position type to apply correct SL direction
   ...
   {
      double targetSL = isBuy ? side.openPrice + InpLockSL3 : side.openPrice - InpLockSL3;
      if(isBuy && side.sl < targetSL) newSL = targetSL;
      if(!isBuy && (side.sl == 0 || side.sl > targetSL)) newSL = targetSL;
   }
   else if(profit >= InpProfitLock2)
   {
      double targetSL = isBuy ? side.openPrice + InpLockSL2 : side.openPrice - InpLockSL2;
      if(isBuy && side.sl < targetSL) newSL = targetSL;
      if(!isBuy && (side.sl == 0 || side.sl > targetSL)) newSL = targetSL;
   }
   else if(profit >= InpProfitLock1)
   {
      double targetSL = isBuy ? side.openPrice + InpLockSL1 : side.openPrice - InpLockSL1;
      if(isBuy && side.sl < targetSL) newSL = targetSL;
      if(!isBuy && (side.sl == 0 || side.sl > targetSL)) newSL = targetSL;
   }

   if(newSL > 0 && newSL != side.sl)
   {
...
//+------------------------------------------------------------------+
//| CHECK POSITION HEALTH (DO NOT TOUCH)                             |
//+------------------------------------------------------------------+
...
void OpenHedge(ENUM_POSITION_TYPE origType, double origPnL)
{
   ...
   // NYAO FIX: Guardar magic original directamente. 
   // Trade.RequestMagic() NO EXISTE en CTrade de MQL5.
   ulong originalMagic = (origType == POSITION_TYPE_BUY) ? InpMagicNumber : InpMagicSell;

   if(origType == POSITION_TYPE_BUY)
   {
      // Posición original BUY perdiendo → abrir hedge SELL (magic 304)
      price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      comment = StringFormat("BASKET|HEDGE SELL|OrigTicket=%d", g_buy.ticket);

      Trade.SetExpertMagicNumber(InpMagicHedgeSell);  // Set magic 304
      Trade.Sell(lots, _Symbol, price, 0, 0, comment);
      Trade.SetExpertMagicNumber(originalMagic);      // Restore magic 301
   ...
   {
      // Posición original SELL perdiendo → abrir hedge BUY (magic 303)
      price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      comment = StringFormat("BASKET|HEDGE BUY|OrigTicket=%d", g_sell.ticket);

      Trade.SetExpertMagicNumber(InpMagicHedgeBuy);   // Set magic 303
      Trade.Buy(lots, _Symbol, price, 0, 0, comment);
      Trade.SetExpertMagicNumber(originalMagic);      // Restore magic 302
...
void ManageActiveBasket()
{
   ...
   // Check grid expansion (simplified - add grid if needed)
   // This is where NYAO's grid logic would go, but we keep GLM5's simpler approach
}
...
void ClosePosition(ulong ticket, string reason)
{
   ...
   // NYAO: Reset partial close state when position closes
...
void CloseAllPositions(string reason)
{
   ...
   // NYAO: Reset all partial close states
   g_buy.initialScore = 0;
   g_buy.partialCloseLevel = 0;
   g_sell.initialScore = 0;
   g_sell.partialCloseLevel = 0;
...
//+------------------------------------------------------------------+
//| GET SIGNAL STRENGTH (DO NOT TOUCH)                               |
//+------------------------------------------------------------------+
SignalStrength GetSignalStrength(ENUM_ORDER_TYPE orderType)
{
   // Cache check - same tick, return cached
   // NYAO FIX: Use TimeCurrent() (tick time) instead of TimeTradeServer() (server time)
   // TimeTradeServer() only changes per bar, causing stale signals within the same bar
   datetime currentTick = TimeCurrent();
   if(currentTick == g_lastSignalTick)
   {
      if(orderType == ORDER_TYPE_BUY) return g_cachedBuySignal;
      else return g_cachedSellSignal;
   }
   ...
   // Cache results
   // NYAO FIX: Only cache the requested signal type. Do NOT copy BUY result into SELL cache.
   // The previous code did: g_cachedSellSignal = result (where result was BUY signal)
   // This caused HedgeRequireSignal to compare against wrong signal direction.
   g_lastSignalTick = currentTick;
   if(orderType == ORDER_TYPE_BUY)
...
//+------------------------------------------------------------------+
//| COMPUTE RAW SCORE (DO NOT TOUCH)                                 |
//+------------------------------------------------------------------+
...
//| Throttled: max 1 check per minute to avoid server lag            |
//+------------------------------------------------------------------+
void CheckHighImpactNews()
{
   ...
   datetime from = now - (InpNewsMinutesBefore + 5) * 60;  // 5 min buffer
...
//+------------------------------------------------------------------+
//| CHECK PARTIAL CLOSE                                              |
//| Only acts on ORIGINAL positions (not hedge), not during basket   |
//| Respects profit lock floor (won't close if SL already positive)  |
//+------------------------------------------------------------------+
...
void CheckPartialCloseSide(ScalperSide &side, ENUM_ORDER_TYPE orderType, ENUM_POSITION_TYPE posType)
{
   ...
   // FLOOR: Check if profit lock has positive SL
   double currentSL = PositionGetDouble(POSITION_SL);
   bool hasPositiveSL = false;
   if(posType == POSITION_TYPE_BUY && currentSL > side.openPrice && currentSL > 0)
      hasPositiveSL = true;
   else if(posType == POSITION_TYPE_SELL && currentSL < side.openPrice && currentSL > 0)
      hasPositiveSL = true;
   ...
   if(side.partialCloseLevel == 0 && ratio <= InpPartialCloseL1Signal)
   {
      ...
      if(remaining >= minVol && closeVol >= minVol)
      {
         ...
         LogMessage(2, StringFormat("[PARTIAL L1] Ticket %d | Closed %.3f lots (25%%) | Signal %.1f/%.1f (ratio=%.2f)",
                     side.ticket, closeVol, st.finalScore, side.initialScore, ratio));
      }
   }
   ...
   else if(side.partialCloseLevel == 1 && ratio <= InpPartialCloseL2Signal)
   {
      ...
      if(remaining >= minVol && closeVol >= minVol)
      {
         ...
         LogMessage(2, StringFormat("[PARTIAL L2] Ticket %d | Closed %.3f lots (50%% of remaining) | Signal %.1f/%.1f (ratio=%.2f)",
                     side.ticket, closeVol, st.finalScore, side.initialScore, ratio));
      }
   }
   ...
   {
      // Respect profit lock: if SL is already positive, don't close, just mark as done
      if(hasPositiveSL)
      {
         LogMessage(2, StringFormat("[PARTIAL L3 BLOCKED] Ticket %d | Profit lock active (SL=%.2f vs Open=%.2f) | Let trailing manage exit",
                     side.ticket, currentSL, side.openPrice));
         side.partialCloseLevel = 3; // Mark as resolved, don't evaluate again
         return;
      }

      ClosePosition(side.ticket, "PARTIAL_L3_SIGNAL_DECAY");
      side.partialCloseLevel = 3;
      LogMessage(2, StringFormat("[PARTIAL L3] Ticket %d | Full close | Signal %.1f/%.1f (ratio=%.2f)",
                  side.ticket, st.finalScore, side.initialScore, ratio));
   }
}
