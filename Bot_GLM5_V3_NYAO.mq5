...
void CheckProfitLock(ScalperSide &side)
{
   ...
   if(profit >= InpProfitLock3 && side.sl < side.openPrice + InpLockSL3)
      newSL = side.openPrice + InpLockSL3;
   else if(profit >= InpProfitLock2 && side.sl < side.openPrice + InpLockSL2)
      newSL = side.openPrice + InpLockSL2;
   else if(profit >= InpProfitLock1 && side.sl < side.openPrice + InpLockSL1)
      newSL = side.openPrice + InpLockSL1;

   if(newSL > 0 && newSL != side.sl)
   {
      Trade.PositionModify(side.ticket, newSL, side.tp);
      LogMessage(3, StringFormat("[PROFIT LOCK] Ticket %d | SL moved to %.2f | Profit: %.2f",
                  side.ticket, newSL, profit));
   }
}
...
void OpenHedge(ENUM_POSITION_TYPE origType, double origPnL)
{
   ...
   if(origType == POSITION_TYPE_BUY)
   {
      ...
      LogMessage(2, StringFormat("[HEDGE OPEN] SELL | Lots=%.2f | Price=%.2f | OrigProfit=%.2f",
                  lots, price, origPnL));
   }
   else
   {
      price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      magic = InpMagicHedgeBuy;
      comment = StringFormat("BASKET|HEDGE BUY|OrigTicket=%d", g_sell.ticket);
      Trade.Buy(lots, _Symbol, price, 0, 0, comment);
      g_basketActive = true;
      g_basketOpenProfit = origPnL;
      LogMessage(2, StringFormat("[HEDGE OPEN] BUY | Lots=%.2f | Price=%.2f | OrigProfit=%.2f",
                  lots, price, origPnL));
   }
}
...
SignalStrength GetSignalStrength(ENUM_ORDER_TYPE orderType)
{
   ...
   datetime currentTick = TimeTradeServer();
   ...
   {
      // Compute sell signal too for cache
      SignalStrength sellResult = result;
      // ... (simplified - in real implementation compute both)
      g_cachedSellSignal = sellResult;
   }
