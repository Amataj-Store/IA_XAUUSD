...
input double Inp_ADX_Threshold      = 23.0;         // Umbral ADX
...
input double Inp_Risk_Per_Trade     = 0.5;          // Riesgo % por trade
...
input double Inp_Max_DD             = 10.0;         // Max drawdown % absoluto
...
input double Inp_Body_Ratio_Min     = 0.60;         // Min body/range
   ...
   if(g_H1_Bull)
   {
      ...
      if(bos || choch)
      {
            ...
            !IsFalseBreak_Long(ref_level))
         {
            ...
            double lot = CalculateLotByRisk(sl_dist, Inp_Risk_Per_Trade);
            if(lot > 0.0)
            {
...
void UpdateH1Bias()
{
   ...
   //--- Detect HH/HL sequence (bullish)
   // Need: High, then Low, then Higher High, then Higher Low
   bool has_hh_hl_sequence = false;

   for(int h1 = 0; h1 < ArraySize(g_SwingHighs) - 1; h1++)
   {
      for(int h2 = h1 + 1; h2 < ArraySize(g_SwingHighs); h2++)
      {
         if(g_SwingHighs[h2].price > g_SwingHighs[h1].price) // HH found
         {
            // Find a Low between these two highs
            for(int l1 = 0; l1 < ArraySize(g_SwingLows); l1++)
            {
               // Low must be between the two highs in time
               if(g_SwingLows[l1].time < g_SwingHighs[h1].time &&
                  g_SwingLows[l1].time > g_SwingHighs[h2].time)
               {
                  // Find previous low for HL comparison
                  for(int l2 = l1 + 1; l2 < ArraySize(g_SwingLows); l2++)
                  {
                     if(g_SwingLows[l2].time > g_SwingLows[l1].time &&
                        g_SwingLows[l2].price > g_SwingLows[l1].price)
                     {
                        has_hh_hl_sequence = true;
                        break;
                     }
                  }
               }
               if(has_hh_hl_sequence) break;
            }
         }
         if(has_hh_hl_sequence) break;
      }
   ...
   //--- Detect LH/LL sequence (bearish)
   // Need: High, then Low, then Lower High, then Lower Low
   ...
   for(int h1 = 0; h1 < ArraySize(g_SwingHighs) - 1; h1++)
   {
      for(int h2 = h1 + 1; h2 < ArraySize(g_SwingHighs); h2++)
      {
         if(g_SwingHighs[h2].price < g_SwingHighs[h1].price) // LH found
         {
            // Find a Low between these two highs
            for(int l1 = 0; l1 < ArraySize(g_SwingLows); l1++)
            {
               if(g_SwingLows[l1].time < g_SwingHighs[h1].time &&
                  g_SwingLows[l1].time > g_SwingHighs[h2].time)
               {
                  // Find next low for LL comparison
                  for(int l2 = l1 + 1; l2 < ArraySize(g_SwingLows); l2++)
                  {
                     if(g_SwingLows[l2].time > g_SwingLows[l1].time &&
                        g_SwingLows[l2].price < g_SwingLows[l1].price)
                     {
                        has_lh_ll_sequence = true;
                        break;
                     }
                  }
               }
               if(has_lh_ll_sequence) break;
            }
         }
         if(has_lh_ll_sequence) break;
      }
   ...
   g_H1_Bull = above_sma && slope_up && has_hh_hl_sequence;
   g_H1_Bear = below_sma && slope_down && has_lh_ll_sequence;
}
...
bool DetectCHoCH_Long(double &reference_level)
{
   ...
   for(int i = 0; i < ArraySize(g_SwingLows) - 1; i++)
   {
      ...
      {
         if(i < ArraySize(g_SwingHighs) && g_SwingHighs[i].price < g_SwingHighs[i + 1].price)
...
bool DetectCHoCH_Short(double &reference_level)
{
   ...
   for(int i = 0; i < ArraySize(g_SwingHighs) - 1; i++)
   {
      ...
      {
         if(i < ArraySize(g_SwingLows) && g_SwingLows[i].price > g_SwingLows[i + 1].price)
...
bool CheckADXStrength()
{
   ...
   double adx_main[2];
   if(CopyBuffer(h_ADX, 0, 1, 2, adx_main) < 2) return false;
   return (adx_main[0] >= Inp_ADX_Threshold) && (adx_main[0] > adx_main[1]);
}
...
bool CheckPause()
{
   ...
   return (daily_loss >= Inp_Daily_Loss_Limit) || (weekly_loss >= Inp_Weekly_Loss_Limit);
}
...
void ManageOpenPositions(double swing_high, double swing_low, double adx_current)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      {
         // Only invalidate by structure if swing_low is valid
         if(swing_low > 0.0 && iClose(_Symbol, PERIOD_M5, 0) < swing_low)
            invalidate = true;
      ...
      {
         // Only invalidate by structure if swing_high is valid
         if(swing_high > 0.0 && iClose(_Symbol, PERIOD_M5, 0) > swing_high)
            invalidate = true;
